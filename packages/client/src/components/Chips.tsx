import { getCardDef, getStatusDef, getSummonDef, type CardView, type StatusInstance, type Summon } from "@baston/engine";
...
export function StatusChips({ statuses }: { statuses: StatusInstance[] }) {
  ...
  return (
    <ul className="chips" aria-label="Effets actifs">
      {statuses.map((s) => {
        const def = getStatusDef(s.defId);
